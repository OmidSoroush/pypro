...
from .forms import PysparkPostForm
...
class PostListView(ListView):
    model = PysparkPost
    template_name = 'pyspark/post_list.html'
    context_object_name = 'posts'
    ordering = ['created_at']

    def get_queryset(self):
        return PysparkPost.objects.filter(published_date__lte=timezone.now())
...
class PostListView2(ListView):
    model = PysparkPost
    template_name = 'pyspark/post_list2.html'
    context_object_name = 'posts'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Add in a QuerySet
        context['single_posts'] = PysparkPost.objects.get(slug=self.kwargs.get('slug'))
        return context


class PostDetailView(DetailView):
    model = PysparkPost
    template_name = 'pyspark/pyspark_detail.html'
    context_object_name = 'post_contents'

    # get a list of all (by default DetailView provides singlewise data)
    def get_context_data(self, *args, **kwargs):
        context = super(PostDetailView, self).get_context_data(*args, **kwargs)
        context['posts'] = PysparkPost.objects.all().order_by('created_at')
...
class CreatePostView(SuperUserRequiredMixin, CreateView):
    login_url = '/login/'
    redirect_field_name = 'pyspark/pyspark_detail.html'
    form_class = PysparkPostForm
    model = PysparkPost
...
class PostDeleteView(SuperUserRequiredMixin,DeleteView):
    model = PysparkPost
    success_url = reverse_lazy('pyspark:post_list')
    # def get_success_url(self, **kwargs):
    #     # obj = form.instance or self.object
    #     return reverse_lazy("pyspark:post_list", kwargs={'slug': self.object.slug})
...
class DraftListView(SuperUserRequiredMixin,ListView):
    login_url = '/login/'
    redirect_field_name = 'pyspark/pyspark_draft_list.html'
    model = PysparkPost
    template_name = 'pyspark/pyspark_draft_list.html'


    def get_context_data(self, *args, **kwargs):
        context = super(DraftListView, self).get_context_data(*args, **kwargs)
        context['drafts'] = PysparkPost.objects.filter(published_date__isnull=True).order_by('created_at')
        return context


@login_required
def post_publish(request, pk):
    post = get_object_or_404(PysparkPost, pk=pk)
    post.publish()
    return redirect('pyspark:single-detail', pk=pk)
