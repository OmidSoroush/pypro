...
class PostListView(ListView):
    model = SQLPost
    template_name = 'sql/post_list.html'
    context_object_name = 'posts'
    ordering = ['created_at']

    def get_queryset(self):
        return SQLPost.objects.filter(published_date__lte=timezone.now())
...
class PostListView2(ListView):
    model = SQLPost
    template_name = 'sql/post_list2.html'
    context_object_name = 'posts'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Add in a QuerySet
        context['single_posts'] = SQLPost.objects.get(slug=self.kwargs.get('slug'))
        return context


class PostDetailView(DetailView):
    model = SQLPost
    template_name = 'sql/sql_detail.html'
    context_object_name = 'post_contents'

    # get a list of all (by default DetailView provides singlewise data)
    def get_context_data(self, *args, **kwargs):
        context = super(PostDetailView, self).get_context_data(*args, **kwargs)
        context['posts'] = SQLPost.objects.all().order_by('created_at')
...
class CreatePostView(SuperUserRequiredMixin, CreateView):
    login_url = '/login/'
    redirect_field_name = 'sql/sql_detail.html'
    form_class = SQLPostForm
    model = SQLPost
...
class PostDeleteView(SuperUserRequiredMixin,DeleteView):
    model = SQLPost
    success_url = reverse_lazy('sql:post_list')
    # def get_success_url(self, **kwargs):
    #     # obj = form.instance or self.object
    #     return reverse_lazy("sql:post_list", kwargs={'slug': self.object.slug})
...
class DraftListView(SuperUserRequiredMixin,ListView):
    login_url = '/login/'
    redirect_field_name = 'sql/sql_draft_list.html'
    model = SQLPost
    template_name = 'sql/sql_draft_list.html'


    def get_context_data(self, *args, **kwargs):
        context = super(DraftListView, self).get_context_data(*args, **kwargs)
        context['drafts'] = SQLPost.objects.filter(published_date__isnull=True).order_by('created_at')
        return context


@login_required
def post_publish(request, pk):
    post = get_object_or_404(SQLPost, pk=pk)
    post.publish()
    return redirect('sql:single-detail', pk=pk)
